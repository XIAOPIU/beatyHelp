...
//    var stringEncoding = NSUTF8StringEncoding
    ...
    func dataFromParameters(parameters: Dictionary<String,AnyObject>) -> NSData {
        ...
        mutData.appendData("--\(boundary)\(multiCRLF)".dataUsingEncoding(NSUTF8StringEncoding))
        var noParams = false
        if notFiles.count == 0 {
            noParams = true
        }
        ...
        for (key,upload) in files {
            ...
            if i == 1 && noParams {
                ...
                mutData.appendData(boundSplit.dataUsingEncoding(NSUTF8StringEncoding))
            }
        }
        if !noParams {
            ...
            mutData.appendData(paramStr.dataUsingEncoding(NSUTF8StringEncoding))
        }
        mutData.appendData("\(multiCRLF)--\(boundary)--\(multiCRLF)".dataUsingEncoding(NSUTF8StringEncoding))
        return mutData
    }
